...
{
    public class Player_Life_Virus_Cond : Condition, Condition.IHasLogic, Condition.IHasValue<ushort>
    {
        public Player_Life_Virus_Cond()
        {
            Type = Condition_Type.Player_Life_Virus;
        }
        ...
        public override void Init(Universal_ConditionEditor uce, Condition start)
        {
            ...
            if (start == null)
                return;
            uce.SetMainValue(1, (start as Player_Life_Virus_Cond).Logic);
            uce.SetMainValue(3, (start as Player_Life_Virus_Cond).Value);
        }
        public override T Parse<T>(object[] input)
        {
            return new Player_Life_Virus_Cond
            {
        ...
        public override string GetFilePresentation(string prefix, int prefixIndex, int conditionIndex)
        {
            if (prefix.Length > 0)
                if (!prefix.EndsWith("_"))
                    prefix += "_";
            string output = "";
            output += ($"{prefix}{(prefix.Length > 0 ? $"{prefixIndex.ToString()}_" : "")}Condition_{conditionIndex}_Type Player_Life_Virus");
